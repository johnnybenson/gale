        ...
        if b[i] == b'@' {
            ...
            if kw == "media" {
                ...
                while i < len && !(b[i] == b'{' && depth == 0) {
                    ...
                    if b[i] == b'"' || b[i] == b'\'' {
                        let q = b[i]; i += 1;
                        while i < len && b[i] != q { if b[i] == b'\\' { i += 1; } i += 1; }
                    }
                    i += 1;
                }
                let me = i;
            ...
            if kw == "import" {
                ...
                if i + 4 <= len && source[i..].to_ascii_lowercase().starts_with("url(") {
                    ...
                    if i < len && (b[i] == b'"' || b[i] == b'\'') {
                        ...
                        while j < len && depth > 0 {
                            if b[j] == b'(' { depth += 1; }
                            else if b[j] == b')' { depth -= 1; if depth == 0 { break; } }
                            j += 1;
                        }
                        let content = &source[us..j];
                        if content.bytes().next().map_or(false, |c| c.is_ascii_digit() || c == b'$') {
...
fn find_param_end(source: &str, start: usize, init_depth: i32) -> usize {
    ...
    while i < len {
        match b[i] {
            b'(' => depth += 1,
            b')' => { if depth <= init_depth { return i; } depth -= 1; }
...
fn check_media_values(
    mt: &str, base: usize,
    rule: &ScssOperatorNoUnspaced, diags: &mut Vec<Diagnostic>,
) {
    let b = mt.as_bytes();
    let len = b.len();
    let mut i = 0;
        ...
        if b[i] == b'(' {
            ...
            while i < len && depth > 0 {
                if b[i] == b'(' { depth += 1; }
                    ...
                    let mut d = 1i32;
                    let mut end = vs;
                    while end < len { if b[end] == b'(' { d += 1; } else if b[end] == b')' { d -= 1; if d == 0 { break; } } end += 1; }
...
{
    check_value_inner(value, base_offset, slash_is_sep, in_interp, in_interp, rule, diags);
}
...
fn check_value_inner(
    ...
    while i < len {
        ...
        if ch == b'(' { paren_depth += 1; i += 1; continue; }
        if ch == b')' {
            if in_calc && paren_depth == calc_depth { in_calc = false; }
            if in_url && paren_depth == url_depth { in_url = false; }
            paren_depth -= 1;
            i += 1;
            continue;
        }
        if in_calc && paren_depth >= calc_depth { i += 1; continue; }
        ...
        if ch == b'=' && i + 1 < len && b[i + 1] == b'=' {
            ...
            if !ok { emit(rule, diags, base_offset + i, 2, "=="); }
            i += 2; continue;
        }
        if ch == b'!' && i + 1 < len && b[i + 1] == b'=' {
            let ok = (i > 0 && b[i - 1] == b' ') && (i + 2 < len && b[i + 2] == b' ');
        ...
        if ch == b'<' && is_comparison_ctx(b, i) {
            ...
            if !ok { emit(rule, diags, base_offset + i, 1, "<"); }
            i += 1; continue;
        }
        if ch == b'>' && is_comparison_ctx(b, i) {
            let ok = (i > 0 && b[i - 1] == b' ') && (i + 1 < len && b[i + 1] == b' ');
        ...
        if ch == b'+' || ch == b'-' || ch == b'*' || ch == b'/' {
            ...
            if ch == b'-' && i + 1 < len && b[i + 1] == b'-' {
                i += 2;
                while i < len && (b[i].is_ascii_alphanumeric() || b[i] == b'-' || b[i] == b'_') { i += 1; }
                continue;
            }
            ...
            // Unary
            if (ch == b'+' || ch == b'-') && is_unary(b, i) { i += 1; continue; }
                ...
                if ws_before && !ws_after {
                    ...
                    if ch == b'+' {
                        if !expr_ctx && (next.is_ascii_digit() || next == b'.') {
                            i += 1; continue; // `1 +1` → list (only in non-expression context)
                        }
                        emit(rule, diags, base_offset + i, 1, "+");
                        i += 1; continue;
                    }
                    // ch == b'-'
                    if !expr_ctx && (next.is_ascii_digit() || next == b'.') { i += 1; continue; } // negative number → list
                    if next == b'$' {
                        emit(rule, diags, base_offset + i, 1, "-");
                        i += 1; continue;
                    }
                    if next == b'#' {
                        if i + 2 < len && b[i + 2] == b'{' { i += 1; continue; } // -#{} → sign
                        // -#hex: check if prev is hex color
                        if prev_is_hex(b, i) {
                            emit(rule, diags, base_offset + i, 1, "-");
                            i += 1; continue;
                        }
                        i += 1; continue; // -#ffc → list
                    }
                    if next == b'(' {
                        emit(rule, diags, base_offset + i, 1, "-");
                        i += 1; continue;
                    }
                    if next.is_ascii_alphabetic() || next == b'_' { i += 1; continue; } // -ident → list
                    emit(rule, diags, base_offset + i, 1, "-");
                    i += 1; continue;
                }
                // No spaces at all
...
fn emit(rule: &ScssOperatorNoUnspaced, diags: &mut Vec<Diagnostic>, offset: usize, len: usize, op: &str) {
    diags.push(
        Diagnostic::new(rule.name(), format!("Expected spaces around operator '{op}'"))
...
fn is_ident_hyphen(b: &[u8], i: usize) -> bool {
    ...
    if prev == b')' { return false; } // fn()-1 → operator
    if prev == b'}' { return true; } // #{...}-1 → identifier

    // Numeric+unit minus numeric: `5px-3px`, `.1px-1px`, `s.1px-1` → operator
    if next.is_ascii_digit() {
        let mut ts = i - 1;
        while ts > 0 && (b[ts - 1].is_ascii_alphanumeric() || b[ts - 1] == b'.' || b[ts - 1] == b'_' || b[ts - 1] == b'-') {
            ts -= 1;
        }
        // $var-1 → variable name → identifier
        if ts > 0 && b[ts - 1] == b'$' { return true; }
        let token = &b[ts..i];
        // Check if token contains a numeric start or `.digit` pattern
        if !token.is_empty() {
            if token[0].is_ascii_digit() || (token[0] == b'.' && token.len() > 1 && token[1].is_ascii_digit()) {
                return false; // operator
            }
            if token.windows(2).any(|w| w[0] == b'.' && w[1].is_ascii_digit()) {
...
fn prev_ends_unit(b: &[u8], i: usize) -> bool {
    ...
    if p.is_ascii_alphabetic() || p == b'_' || p == b'%' {
        let mut j = i - 1;
        while j > 0 && (b[j - 1].is_ascii_alphanumeric() || b[j - 1] == b'_' || b[j - 1] == b'-' || b[j - 1] == b'.') { j -= 1; }
        if j > 0 && b[j - 1] == b'#' { return false; } // #0f0- → hex color
        return true;
    }
    ...
    if p.is_ascii_digit() {
        let mut j = i - 1;
        while j > 0 && (b[j - 1].is_ascii_alphanumeric() || b[j - 1] == b'_' || b[j - 1] == b'-' || b[j - 1] == b'.') { j -= 1; }
        if j > 0 && b[j - 1] == b'#' { return false; } // hex color
        if j > 0 && b[j - 1] == b'$' { return false; } // variable
        // Check if the token starts with a letter
        if j < i && b[j].is_ascii_alphabetic() { return true; }
    }
    false
}
...
fn prev_is_hex(b: &[u8], i: usize) -> bool {
    let mut j = i;
    while j > 0 && b[j - 1].is_ascii_whitespace() { j -= 1; }
    if j == 0 { return false; }
    let te = j;
    while j > 0 && b[j - 1].is_ascii_alphanumeric() { j -= 1; }
    if j > 0 && b[j - 1] == b'#' {
        let tok = &b[j..te];
...
fn is_fn_call_next(b: &[u8], j: usize) -> bool {
    ...
    let c = b[j];
    if !c.is_ascii_alphabetic() && c != b'_' { return false; }
    let mut k = j;
    while k < len && (b[k].is_ascii_alphanumeric() || b[k] == b'-' || b[k] == b'_' || b[k] == b'.') { k += 1; }
    k < len && b[k] == b'('
}
...
fn in_css_slash_ctx(b: &[u8], i: usize) -> bool {
    ...
    // Scan past the token
    let te = j;
    while j > 0 && (b[j - 1].is_ascii_alphanumeric() || b[j - 1] == b'.' || b[j - 1] == b'_' || b[j - 1] == b'-' || b[j - 1] == b'$' || b[j - 1] == b'}' || b[j - 1] == b')') { j -= 1; }
    // Check for `/` (possibly with spaces)
    let mut k = j;
    while k > 0 && b[k - 1] == b' ' { k -= 1; }
    if k > 0 && b[k - 1] == b'/' {
        let sp = k - 1;
...
enum TokType { Numeric, Variable, Ident, Interp, FnCall, Parens, Hex, Signed, Unknown }

fn classify_before(b: &[u8], i: usize) -> TokType {
    if i == 0 { return TokType::Unknown; }
    let mut j = i;
    while j > 0 && b[j - 1] == b' ' { j -= 1; }
    if j == 0 { return TokType::Unknown; }
    let last = b[j - 1];
    if last == b')' {
        let mut d = 1i32; j -= 1;
        while j > 0 && d > 0 { j -= 1; if b[j] == b')' { d += 1; } else if b[j] == b'(' { d -= 1; } }
        if j > 0 && (b[j - 1].is_ascii_alphanumeric() || b[j - 1] == b'_' || b[j - 1] == b'.') { return TokType::FnCall; }
        return TokType::Parens;
    }
    if last == b'}' { return TokType::Interp; }
    if last == b'%' { return TokType::Numeric; }
    let te = j;
    while j > 0 && (b[j - 1].is_ascii_alphanumeric() || b[j - 1] == b'.' || b[j - 1] == b'_' || b[j - 1] == b'-') { j -= 1; }
    let tok = &b[j..te];
    if tok.is_empty() { return TokType::Unknown; }
    if j > 0 && b[j - 1] == b'#' { return TokType::Hex; }
    if j > 0 && b[j - 1] == b'$' { return TokType::Variable; }
    if tok[0].is_ascii_digit() || (tok[0] == b'.' && tok.len() > 1 && tok[1].is_ascii_digit()) { return TokType::Numeric; }
    if tok[0].is_ascii_alphabetic() { return TokType::Ident; }
    TokType::Numeric
}
...
fn classify_after(b: &[u8], i: usize) -> TokType {
    ...
    if j >= len { return TokType::Unknown; }
    let f = b[j];
    if f == b'+' || f == b'-' { return TokType::Signed; }
    if f == b'#' && j + 1 < len && b[j + 1] == b'{' { return TokType::Interp; }
    if f == b'(' { return TokType::Parens; }
    if f == b'$' { return TokType::Variable; }
    if f == b'#' { return TokType::Hex; }
    if f.is_ascii_digit() || f == b'.' { return TokType::Numeric; }
    if f.is_ascii_alphabetic() || f == b'_' {
        let mut k = j;
        while k < len && (b[k].is_ascii_alphanumeric() || b[k] == b'-' || b[k] == b'_' || b[k] == b'.') { k += 1; }
        if k < len && b[k] == b'(' { return TokType::FnCall; }
...
fn is_css_slash(b: &[u8], i: usize, pd: i32, in_interp: bool) -> bool {
    ...
    if lt == TokType::Variable || rt == TokType::Variable {
        if (lt == TokType::Variable && rt == TokType::Ident) || (lt == TokType::Ident && rt == TokType::Variable) { return true; }
        return false;
    }
    if lt == TokType::FnCall || rt == TokType::FnCall { return false; }
    if rt == TokType::Signed { return false; }
    if lt == TokType::Ident || rt == TokType::Ident { return true; }
    if lt == TokType::Interp || rt == TokType::Interp { return true; }
...
fn has_operator_before_token(b: &[u8], slash_pos: usize) -> bool {
    ...
    while j > 0 && (b[j - 1].is_ascii_alphanumeric() || b[j - 1] == b'.' || b[j - 1] == b'_' || b[j - 1] == b'-' || b[j - 1] == b'%') { j -= 1; }
...
fn is_modulo(b: &[u8], i: usize) -> bool {
    ...
    if sp_before {
        ...
        if ac == b'#' && ap + 1 < len && b[ap + 1] == b'{' { return false; }
        // LHS is interpolation → not modulo
        let mut bj = i;
        while bj > 0 && b[bj - 1] == b' ' { bj -= 1; }
        if bj > 0 && b[bj - 1] == b'}' { return false; }
    ...
    if before.is_ascii_digit() || before == b'.' {
        ...
        let sp_after = i + 1 < len && b[i + 1] == b' ';
        if sp_after && b[ap] == b'-' && ap + 1 < len && b[ap + 1] == b'(' { return true; }
        return false;
    }
    if before == b')' { return true; }
    if before == b'}' { return false; }
    // $var%
    let mut j = i - 1;
    while j > 0 && (b[j].is_ascii_alphanumeric() || b[j] == b'-' || b[j] == b'_') { j -= 1; }
    if j < i && b[j] == b'$' { return true; }
    false
}
...
fn is_unary(b: &[u8], i: usize) -> bool {
    if i == 0 { return true; }
    let p = b[i - 1];
    if p == b'(' || p == b',' || p == b':' { return true; }
    let mut j = i;
    while j > 0 && b[j - 1].is_ascii_whitespace() { j -= 1; }
    if j == 0 { return true; }
    let pnw = b[j - 1];
    matches!(pnw, b'+' | b'*' | b'/' | b',' | b'(' | b':' | b'=' | b'<' | b'>')
}
...
fn count_ws_before(b: &[u8], i: usize) -> usize {
    let mut c = 0; let mut j = i;
    while j > 0 && b[j - 1].is_ascii_whitespace() { c += 1; j -= 1; }
    c
}
fn count_ws_after(b: &[u8], i: usize) -> usize {
    let mut c = 0; let mut j = i + 1;
    while j < b.len() && b[j].is_ascii_whitespace() { c += 1; j += 1; }
    c
}
fn has_nl_before(b: &[u8], i: usize) -> bool {
    let mut j = i;
    while j > 0 && b[j - 1].is_ascii_whitespace() { if b[j - 1] == b'\n' { return true; } j -= 1; }
    false
}
fn has_nl_after(b: &[u8], i: usize) -> bool {
    let mut j = i + 1;
    while j < b.len() && b[j].is_ascii_whitespace() { if b[j] == b'\n' { return true; } j += 1; }
    false
}
...
mod tests {
    ...
    fn scss_context() -> RuleContext<'static> {
        RuleContext { file_path: "test.scss", source: "", syntax: Syntax::Scss, options: None }
    }
    fn css_context() -> RuleContext<'static> {
        RuleContext { file_path: "test.css", source: "", syntax: Syntax::Css, options: None }
    }
    ...
    fn skips_calc_operators() {
        ...
        assert!(rule.check(&node, &scss_context()).is_empty(), "calc operators should be skipped");
    }
    #[test]
